...
// The backend tests select everything; test_dsa_kpool sets index_topk = DSA_INDEXER_TOP_K,
// so index_topk/index_kpool = 2 whole pools are selected per query row.
...
static gguf_context_ptr get_gguf_ctx(const llm_arch arch, const bool moe, const bool nextn = false) {
    ...
    //       (test_dsa_kpool overrides it with DSA_INDEXER_TOP_K to get a real cut)
...
static int test_dsa_kpool(const size_t seed, const float stdev, const int verbosity) {
    struct user_data_t {
        struct {
            ggml_log_callback callback;
            void * user_data;
    ...
    {
        // get_gguf_ctx selects everything by default, so no cut would ever be made
        llama_model_saver ms(LLM_ARCH_GLM5NEXT, gguf_ctx.get());
    ...
    for (const auto & dc : dev_configs) {
        ...
        auto model_and_ctx = get_model_and_ctx(gguf_ctx.get(), nullptr, seed, stdev, dc.first,
...
//
static int test_mtp(const size_t seed, const float stdev, const int verbosity) {
    struct user_data_t {
        struct {
            ggml_log_callback callback;
            void * user_data;
    ...
    for (const auto & dc : dev_configs) {
        ...
        if (file != nullptr && llama_model_saver_supports_arch(LLM_ARCH_GLM5NEXT)) {
            ...
            rewind(file);
            auto rt_mtp = get_model_and_ctx(nullptr, file, seed, stdev, dc.first,
                LLAMA_SPLIT_MODE_LAYER, false, nullptr, nullptr, /*load_mtp =*/ true);
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        int ret = 0;
        if (arch_matches(arch_filter, LLM_ARCH_GLM5NEXT)) {
            ret |= test_dsa_kpool(seed, stdev, verbosity);
            ret |= test_mtp(seed, stdev, verbosity);
        }
        ret |= test_backends(arch_filter, seed, stdev, verbosity);
        return ret;
